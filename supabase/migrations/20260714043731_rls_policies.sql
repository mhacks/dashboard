...
CREATE POLICY "hacker_applicants_update_own_or_organizer" ON "hacker_applicants" AS PERMISSIVE FOR UPDATE TO "authenticated" USING ("hacker_applicants"."user_id" = (select auth.uid()) OR exists (
  select 1
  from public.users
  where id = (select auth.uid())
    and role = 'organizer'
)) WITH CHECK ("hacker_applicants"."user_id" = (select auth.uid()) OR exists (
  select 1
  from public.users
  where id = (select auth.uid())
    and role = 'organizer'
...
CREATE POLICY "hacker_application_review_events_organizer_all" ON "hacker_application_review_events" AS PERMISSIVE FOR ALL TO "authenticated" USING (exists (
  select 1
  from public.users
  where id = (select auth.uid())
    and role = 'organizer'
...
CREATE POLICY "hacker_application_reviews_organizer_all" ON "hacker_application_reviews" AS PERMISSIVE FOR ALL TO "authenticated" USING (exists (
  select 1
  from public.users
  where id = (select auth.uid())
    and role = 'organizer'
