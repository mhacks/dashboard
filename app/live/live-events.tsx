  ...
  return (
    <section aria-label="Live predictions" className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="font-red-hat text-sm font-semibold uppercase tracking-[0.18em] text-olive/80">
          Live Predictions
        </h2>
      ...
      >
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {active.status === "open" ? (
              <span className="font-red-hat rounded-full bg-[#c43a1c]/18 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wider text-[#86250f]">
                Betting open
              </span>
        ...
        {active.status === "open" && !myBet ? (
          <div className="mt-4 space-y-3">
            ...
            <p className="font-red-hat text-[11px] font-medium uppercase tracking-wider text-ink/65">
              {selected
...
function QuickLinks() {
  return (
    <nav aria-label="Quick links" className="flex flex-wrap gap-2 sm:gap-3">
    ...
    <section aria-label="Currency system" className="space-y-3">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="font-heading mt-1 text-base leading-6 text-ink/55">
      ...
      <div className="grid gap-3 lg:grid-cols-[1.15fr_0.85fr]">
        <div className={cn(LIQUID_GLASS_PANEL_CLASS, "rounded-3xl p-5")}>
          ...
          <div className="grid gap-2 sm:grid-cols-3">
            {CURRENCY_EARNERS.map((item) => (
              <article
                key={item.label}
                className="rounded-2xl border border-olive/10 bg-white/45 p-4"
              >
                <div className="font-heading text-2xl text-olive tabular-nums">
                  {item.reward}
                </div>
                <h4 className="font-red-hat mt-2 text-sm font-semibold text-ink">
                  {item.label}
                </h4>
                <p className="font-red-hat mt-1 text-xs leading-5 text-ink/58">
                  {item.detail}
                </p>
              </article>
            ))}
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
          <div className={cn(LIQUID_GLASS_CARD_CLASS, "rounded-3xl p-5")}>
            ...
            <div className="space-y-2">
              {CURRENCY_REWARDS.map((reward) => (
                <div
                  key={reward.item}
                  className="flex items-center justify-between gap-3 rounded-2xl bg-white/45 px-3 py-2"
                >
                  <span className="font-red-hat text-sm font-medium text-ink">
                    {reward.item}
                  </span>
                  <span className="font-heading text-sm text-olive tabular-nums">
                    {reward.price}
                  </span>
                </div>
              ))}
              ...
              {LEADERBOARD_PREVIEW.map((team, index) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="font-red-hat text-[11px] text-ink/45">
                      {team.delta} today
                    </div>
                  </div>
                  <span className="font-heading text-sm text-olive tabular-nums">
...
function NextEventCountdown({ events }: { events: LiveEvent[] }) {
  ...
  return (
    <div
      className={cn(
        LIQUID_GLASS_PILL_CLASS,
        "liquid-glass-surface-strong inline-flex w-fit items-center gap-3 rounded-full border-olive/15 bg-paper/90 px-4 py-2 shadow-[0_10px_30px_-14px_rgba(31,42,22,0.45)]",
      )}
    >
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto flex max-w-6xl flex-col gap-6 px-5 pt-12 pb-20 sm:px-8 sm:pt-20 sm:pb-28 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="font-heading mt-3 max-w-xl text-lg font-medium leading-7 text-white drop-shadow-[0_2px_10px_rgba(0,0,0,0.8)] sm:text-xl">
          ...
          <div className="flex flex-col items-start gap-3 lg:items-end">
            <div
              className={cn(
                LIQUID_GLASS_PILL_CLASS,
                "liquid-glass-surface-strong font-heading inline-flex w-fit items-center gap-2.5 rounded-full border-olive/15 bg-paper/90 px-4 py-2 text-base text-olive shadow-[0_10px_30px_-14px_rgba(31,42,22,0.45)]",
              )}
            >
      ...
      <section className="relative mx-auto -mt-10 max-w-5xl space-y-8 px-5 pb-20 sm:px-8">
        ...
        {searchActive ? (
          ...
          <div className="space-y-4 text-center">
            <p className="font-red-hat text-ink/55">No upcoming events left.</p>
            {allDays.length > 0 ? (
              <button
                type="button"
