...
export function Footer({ variant = "default" }: { variant?: "default" | "compact" }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative z-10 flex flex-col justify-center px-6 md:px-[8vw]",
          compact
            ? "py-8 md:py-10"
            : "min-h-[240px] py-14 md:min-h-[280px] md:py-16",
        )}
      >
        {/* Logo / pages / rights */}
        <div
          className={cn(
            "grid items-center md:grid-cols-[1fr_auto_1fr]",
            compact ? "gap-5" : "gap-8",
          )}
        >
          <Logo
          ...
          <nav
            className={cn(
              "flex flex-wrap justify-center gap-x-6 gap-y-3 [text-shadow:0_1px_12px_rgba(29,36,18,0.45)] md:gap-8",
